...
const FlashFwdConfig& default_config(int64_t head_dim) {
    const std::string preferred = "hd" + std::to_string(head_dim) + "_bm64_bn64_wm4_wn2_s2";
    const FlashFwdConfig* config = find_config(preferred);
    if (config != nullptr) {
        return *config;
    }
...
PYBIND11_MODULE(flashatten_v3_sm120, m) {
    m.doc() = "pybind wrapper for flashattention/v3_mMnNkN/flashatten_v3.hpp";
