...
TRAITS_RE = re.compile(
    ...
    r"ELi(?P<block_m>\d+)ELi(?P<block_n>\d+)ELi(?P<warps_m>\d+)"
    r"ELi(?P<warps_n>\d+)ELi(?P<stage>\d+)E"
)
...
def readable_func_name(func_name):
    ...
    return (
        ...
        f"_wm{values['warps_m']}_wn{values['warps_n']}_s{values['stage']}"
    )
...
def main():
    ...
    for line in args.input.read_text(errors="replace").splitlines():
        ...
        if match and current_stack is not None and current_spill is not None:
            ...
            rows.append(f"{current_func} {current_spill} {current_stack} {registers}")
            current_func = None
            current_stack = None
            current_spill = None

    header = "func_name spill stack registers"
