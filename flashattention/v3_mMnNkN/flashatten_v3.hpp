...
dv_flag void O_r2s_copy(auto& cpy_atom, auto& mma, auto& src, auto& dst, int tid) {
...
dv_flag void O_r2s(auto& cpy_atom, auto& mma, auto& src, auto& dst, int tid) {
    O_r2s_copy(cpy_atom, mma, src, dst, tid);
    tma_store_fence();
}

dv_flag void O_s2r(auto& cpy_atom, auto& mma, auto& src, auto& dst, int tid) {
    auto s2r = make_tiled_copy_C(cpy_atom, mma);
    auto thr_s2r = s2r.get_slice(tid);
    Tensor cpy_src = thr_s2r.partition_S(src);
    Tensor cpy_dst = thr_s2r.retile_D(dst);
    copy(s2r, cpy_src, cpy_dst);
}
...
template<int kBlockN, int kBlockM, int WARPS_M, int WARPS_N>
dv_flag void causal_mask(auto& tensor, int q_block, int i) {
    constexpr int M_len = kBlockM/WARPS_M;
    constexpr int N_len = kBlockN/WARPS_N;
    int warp_row = ((threadIdx.y-4)%WARPS_M)*M_len;
    int warp_col = ((threadIdx.y-4)/WARPS_M)*N_len;
    int row_offset = q_block*kBlockM + warp_row  + threadIdx.x/4;
    int col_offset = i*kBlockN + warp_col + (threadIdx.x%4)*2;
...
template<int kBlockM, int WARPS_M, int WARPS_N>
dv_flag void normalize_stats(auto& sstorage, auto& rS_max, auto& rS_sum, auto& rO_RC, float scale) {
    constexpr int kNWarps = WARPS_M * WARPS_N;
    const int wid = threadIdx.y - 4;
    const int warp_m = wid % WARPS_M;
    const int warp_n = wid / WARPS_M;
    const int lane_quad = threadIdx.x % 4;
    const int lane_row = threadIdx.x / 4;

    if constexpr (WARPS_N > 1) {
        CUTE_UNROLL
        for(int r=0; r<size<0>(rO_RC); ++r) {
            const int row = warp_m * (kBlockM / WARPS_M) + lane_row + r * 8;
            const float row_sum = flash::warp_reduce_sum<4>(rS_sum(r));
            if(lane_quad == 0) {
                sstorage.row_max[warp_n * kBlockM + row] = rS_max(r);
                sstorage.row_sum[warp_n * kBlockM + row] = row_sum;
            }
        }
        bar_sync<kNWarps>();
        ...
        for(int r=0; r<size<0>(rO_RC); ++r) {
            ...
            CUTE_UNROLL
            for(int n=0; n<WARPS_N; ++n) {
                global_max = fmaxf(global_max, sstorage.row_max[n * kBlockM + row]);
            }

            float global_sum = 0.f;
            CUTE_UNROLL
            for(int n=0; n<WARPS_N; ++n) {
                float m = sstorage.row_max[n * kBlockM + row];
                float s = sstorage.row_sum[n * kBlockM + row];
                global_sum += s * exp2f((m - global_max) * scale);
            }

            float partial_scale = rS_max(r) == -INFINITY ? 0.f : exp2f((rS_max(r) - global_max) * scale);
            float inv_sum = global_sum == 0.f ? 1.f : __fdividef(partial_scale, global_sum);
            CUTE_UNROLL
...
template<class T, int WARPS_M, int WARPS_N>
dv_flag void reduce_o_warps_n(auto& sstorage, auto& rO, auto& mma_pv,
                              auto& r2s_o, auto& s2r_o, int tid) {
    if constexpr (WARPS_N > 1) {
        constexpr int kNWarps = WARPS_M * WARPS_N;
        const int wid = threadIdx.y - 4;
        const int warp_n = wid / WARPS_M;
        ...
        if(warp_n > 0) {
            Tensor rO_f16 = convert_f2h<T>(rO);
            auto sO_partial = sstorage.get_sO(warp_n - 1);
            O_r2s_copy(r2s_o, mma_pv, rO_f16, sO_partial, tid);
        }
        cutlass::arch::NamedBarrier::sync(kNWarps * 32, cutlass::arch::ReservedNamedBarriers::EpilogueBarrier);

        if(warp_n == 0) {
            CUTE_UNROLL
            for(int n=1; n<WARPS_N; ++n) {
                auto sO_partial = sstorage.get_sO(n - 1);
                auto rO_partial = make_tensor<T>(shape(rO));
                O_s2r(s2r_o, mma_pv, sO_partial, rO_partial, tid);
                CUTE_UNROLL
                for(int i=0; i<size(rO); ++i) {
                    rO(i) += static_cast<float>(rO_partial(i));
                }
            }
        }
    }
...
gemm_flash_v(auto& sstorage, auto& s2r_B, int tid,
    ...
    CUTE_UNROLL
    for(int i=0; i<K; ++i) {
        copy(s2r_B, src_B(_, _, i), dst_B(_, _, i));
        cute::gemm(tiled_mma, rA(_, _, i), rB(_, _, i), rC);
    }
    pipe.release();
}
...
template<class T, int WARPS_M, int WARPS_N>
dv_flag void O_store(auto& sstorage, auto& rO, auto& mma_pv,
                     auto& r2s_o, auto& s2r_o,
                     auto& sO, int tid, int warp_n) {
    reduce_o_warps_n<T, WARPS_M, WARPS_N>(sstorage, rO, mma_pv, r2s_o, s2r_o, tid);
    if constexpr (WARPS_N == 1) {
        Tensor rO_f16 = convert_f2h<T>(rO);
...
__global__ void 
__launch_bounds__((kNWarps+4)*32, 1)
flash_atten_v3_fwd(    
    float scale,
    int seq_len,
    int batch,
    ...
    constexpr int WARPS_M  = CONFIG_PARAMS::WARPS_M;
    constexpr int WARPS_N  = CONFIG_PARAMS::WARPS_N;
    constexpr int kProducerRegCount = 40;
    constexpr int kConsumerRegCount = get_consumer_registers<kNWarps, kProducerRegCount>();
    const int M_num = CEILDIV(seq_len, kBlockM);
    const int N_num = CEILDIV(seq_len, kBlockN);
    const int tiles_num = M_num*batch*kHeadNum;  
    using LayoutQ = typename SStorage::LayoutQ;
    using LayoutK = typename SStorage::LayoutK;
    using LayoutV = typename SStorage::LayoutV;
    using LayoutO = typename SStorage::LayoutO;
    constexpr int STAGE_V = SStorage::v_stage;
    ...
    if(producer<4>()) {
        ...
        SimplePipeline<STAGE_V,sV_bytes> pipeline_v(&(sstorage.p_v), true);
        for(int tiled_id=blockIdx.x; tiled_id<tiles_num; tiled_id+=gridDim.x) {
            if(threadIdx.x==0 && threadIdx.y==0) {
                ...
                for(int i=0; i<k_blocks; ++i) {
                    pipeline_qk.copy_g2s<sK_bytes>(tma_k, tma_gK(_, i), tma_sK(_, pipeline_qk.index()));
        ...
        SimplePipeline<STAGE_V,sV_bytes> pipeline_v(&(sstorage.p_v), false);
        const int wid = threadIdx.y-4;
        ...
        Copy_Atom<SM75_U32x4_LDSM_N, T> s2r_o;
        ...
        for(int tiled_id=blockIdx.x; tiled_id<tiles_num; tiled_id+=gridDim.x) {
            ...
            for(int i=0; i<k_blocks; ++i) {
                ...
                if constexpr(is_causal) {
                    if((i+1)*kBlockN > q_block*kBlockM) 
                        causal_mask<kBlockN, kBlockM, WARPS_M, WARPS_N>(rS_RC, q_block, i);
                }
            ...
            normalize_stats<kBlockM, WARPS_M, WARPS_N>(sstorage, rS_max, rS_sum, rO_RC, scale);
            auto sO = sstorage.get_sO();
            O_store<T, WARPS_M, WARPS_N>(
                sstorage, rO, mma_pv, r2s_o, s2r_o,
                sO, tid, warp_n);
...
template<typename T, bool is_causal,
         ...
         int kBlockM, int kBlockN, int WARPS_M, int WARPS_N, int STAGE>
void flash_fwd(
    float scale,
    int seq_len,
    int batch,
    ...
    using CONFIG_PARAMS = Flash_fwd_kernel_traits<
        kHeadNum, kHeadDim,
        kBlockM, kBlockN, WARPS_M, WARPS_N, STAGE, T>;
    constexpr int kNWarps = CONFIG_PARAMS::kNWarps;
