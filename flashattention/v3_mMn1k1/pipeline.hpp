...
public:
    ...
    uint64_t* full_barrier = nullptr;
    uint64_t* empty_barrier = nullptr;
    uint32_t phase = 0;

    dv_flag SimplePipeline() = delete;
    dv_flag SimplePipeline(PipelineStorage<1>* storage_, bool producer = false)
        : full_barrier(&storage_->full_barrier[0])
        , empty_barrier(&storage_->empty_barrier[0])
        , phase(producer ? 1u : 0u) {}

    template <uint32_t Bytes = TransactionBytes>
    dv_flag void acquire() {
        static_assert(Bytes > 0, "TransactionBytes must be non-zero");
        EmptyBarrier::wait(empty_barrier, phase);
        FullBarrier::arrive_and_expect_tx(full_barrier, Bytes);
    }

    template <uint32_t Bytes = TransactionBytes, class Tma, class SrcTensor, class DstTensor>
    dv_flag void copy_g2s(Tma const& tma, SrcTensor const& src_tensor, DstTensor&& dst_tensor) {
        acquire<Bytes>();
        ::cute::copy(tma.with(*full_barrier), src_tensor, static_cast<DstTensor&&>(dst_tensor));
        ++(*this);
    }

    dv_flag void wait() {
        FullBarrier::wait(full_barrier, phase);
    }

    dv_flag void release() {
        EmptyBarrier::arrive(empty_barrier);
        ++(*this);
    }
